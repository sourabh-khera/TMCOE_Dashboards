...
query = 'select dest_city, sum(iov) as iov, count(booking_id) as totalBookings, count(no_of_passengers) as totalPassengers from flight_kpi where dest_city is not null group by dest_city  order by iov desc limit 15';
