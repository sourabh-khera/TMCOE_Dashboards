...
topRoutes = [];
query = 'select route, sum(iov) as iov, count(booking_id) as totalBookings, count(no_of_passengers) as totalPassengers from flight_kpi where route is not null group by route order by iov desc limit 10';
