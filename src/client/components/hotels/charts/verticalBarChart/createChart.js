const createChart = (height, width, margin, d3, topHotelsDestinations, node) => {
  const svgNode = node;
  ...
  xScale.domain(topHotelsDestinations.map(d => d.Destination));
  yScale.domain([0, d3.max(topHotelsDestinations, d => d.IBV)]);

  yLineScale.domain([0, d3.max(topHotelsDestinations, d => d.TotalBookings)]);
