const createChart = (height, width, margin, d3, topOrigins, node) => {
  const svgNode = node;
  ...
  const yLine2Scale = d3.scaleLinear()
    .range([height, 0]);
  const color = d3.scaleOrdinal(['#fdc10a']);
  const format = d3.format(',d');
  const lineColor = d3.scaleOrdinal(['#1f78b5', '#6b486b']);
  ...
  xScale.domain(topOrigins.map(d => d.Origin));
  yScale.domain([0, d3.max(topOrigins, d => d.IOV)]);

  yLine1Scale.domain([0, d3.max(topOrigins, d => d.TotalBookings)]);
  yLine2Scale.domain([0, d3.max(topOrigins, d => d.TotalPassengers)]);
